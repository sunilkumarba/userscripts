...
// @copyright    2021+, Sunil Kumar B A
// @license      MIT
// @grant        GM_addStyle
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
  ...
  class DataStore {
    ...
    set(prop, value) {
      ...
      // console.log(`DataStore.set: prop:${prop}, value: ${value}, oldValue:${oldValue}`);

      if (oldValue !== value)
        this.notifyPropertyChange(prop, value);
    }
    ...
    setAll(data) {
      ...
      let oldConfig = this.getAll();

      for (let key in data) {
        GM_setValue(key, data[key]);

        if (!(key in oldConfig) || oldConfig[key] !== data[key])
          this.notifyPropertyChange(key, data[key]);
      }
    }

    registerPropertyChangeListener(property, fnListener) {
      if (!(property in this.listeners))
        this.listeners[property] = [];

      if (typeof fnListener === "function")
        this.listeners[property].push(fnListener);
    }
    ...
    notifyPropertyChange(property, newValue) {
      ...
      this.listeners[property].forEach((listener, i) => {
  ...
  $(function () {
    ...
    function addAutoFetchToggleButton() {
      var iconClass = disableAutoFetchOnScroll ? "square" : "check-square",
        toggleAutoFetchOnScrollButtonHtml =
          "<button id='autoFetchOnScrollToggleBtn'><i class='far fa-" + iconClass + "'></i> Auto Fetch</button>";
      ...
      btnToggleAutoFetchOnScroll.on("click", function (e) {
        stopEvent(e);
        dataStore.set("disableAutoFetchOnScroll", !disableAutoFetchOnScroll);
      });
    ...
    function getUpdatedNewsTitle(titleText) {
      ...
      if (titleText.indexOf('<span class="title-first-word">') >= 0) return titleText;
      ...
      $.ajax({
        ...
        success: function (result) {
          ...
          newsItems.each(function (i) {
            ...
            if (title) title.html(getUpdatedNewsTitle(title.text()));
          });

          pagerEle.before(newsItems);
          ...
          if (nextUrl) nextButtonLink.attr("href", nextUrl);
    ...
    function checkAndFetchNewsItems() {
      if (!disableAutoFetchOnScroll && isPagerVisible()) fetchNewsItems();
    }

    function setupConfigChangeListeners() {
      dataStore.registerPropertyChangeListener("disableAutoFetchOnScroll", (newValue) => {
        // console.log(`disableAutoFetchOnScroll value changed to ${newValue}.`);
        disableAutoFetchOnScroll = newValue;
        updateAutoFetchOnScrollButton();
      });

      dataStore.registerPropertyChangeListener("enableCustomCssStyles", (newValue) => {
        // console.log(`enableCustomCssStyles changed to ${newValue}`);
        enableCustomCssStyles = newValue;
    ...
    function loadConfig() {
      ...
      if (!('disableAutoFetchOnScroll' in config)) {
        dataStore.set('disableAutoFetchOnScroll', false);
        disableAutoFetchOnScroll = false;
      } else
        disableAutoFetchOnScroll = config.disableAutoFetchOnScroll;

      if (!('enableCustomCssStyles' in config)) {
        dataStore.set('enableCustomCssStyles', false);
      }
    }
